...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health":
            ...
            # Strip stocks for list view (send only group summaries)
            summary = {k: {x: v[x] for x in v if x != "stocks"}
                       for k, v in themes.items()}
            payload = json.dumps({"themes": summary}).encode()
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self.send_header("Content-Length", len(payload))
            self.send_cors()
            ...
            # /api/themes/Oil%20%26%20Gas%20E%26P
            from urllib.parse import unquote
            industry = unquote(self.path[len("/api/themes/"):])
            themes   = compute_themes()
            data     = themes.get(industry)
            if data:
